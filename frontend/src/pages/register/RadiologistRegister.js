...
const BG = () => (
  <svg viewBox="0 0 600 800" xmlns="http://www.w3.org/2000/svg"
    ...
    <circle cx="500" cy="600" r="220" fill="none" stroke="#1d9e75" strokeWidth="0.9"/>
    <circle cx="500" cy="600" r="170" fill="none" stroke="#1d9e75" strokeWidth="1.1"/>
    <circle cx="500" cy="600" r="120" fill="none" stroke="#1d9e75" strokeWidth="1.4"/>
    <circle cx="500" cy="600" r="70"  fill="none" stroke="#1d9e75" strokeWidth="1.8"/>
    <circle cx="500" cy="600" r="30"  fill="none" stroke="#1d9e75" strokeWidth="2.2"/>
    <line x1="500" y1="360" x2="500" y2="840" stroke="#1d9e75" strokeWidth="0.5"/>
    <line x1="260" y1="600" x2="740" y2="600" stroke="#1d9e75" strokeWidth="0.5"/>
    ...
    {/* Second ECG bottom */}
    <polyline points="160,740 200,740 212,714 228,766 240,726 254,752 270,740 380,740" fill="none" stroke="#5dcaa5" strokeWidth="1.3"/>
    ...
    {/* X-ray film frame top-right */}
    <rect x="420" y="30" width="130" height="160" rx="4" fill="none" stroke="#1d9e75" strokeWidth="1.1"/>
    <rect x="430" y="40" width="110" height="140" rx="2" fill="none" stroke="#5dcaa5" strokeWidth="0.7"/>
    <ellipse cx="485" cy="90" rx="32" ry="38" fill="none" stroke="#5dcaa5" strokeWidth="0.9"/>
    <line x1="453" y1="90" x2="517" y2="90" stroke="#5dcaa5" strokeWidth="0.6"/>
    <line x1="485" y1="52" x2="485" y2="128" stroke="#5dcaa5" strokeWidth="0.6"/>

    {/* Spine vertebrae — right */}
    {[560,582,604,626].map((y,i)=>(
      <rect key={i} x="550" y={y} width="24" height="16" rx="3" fill="none" stroke="#1d9e75" strokeWidth="0.9"/>
    ))}

    {/* Stethoscope — bottom-left */}
    <path d="M80 660 Q70 650 70 635 Q70 615 90 615 Q110 615 110 635 Q110 652 95 660 Q85 666 85 680 Q85 700 105 700" fill="none" stroke="#1d9e75" strokeWidth="1.3"/>
    <circle cx="108" cy="704" r="6" fill="none" stroke="#1d9e75" strokeWidth="1.2"/>

    {/* Brain outline top-center */}
    <path d="M270 50 Q260 38 268 28 Q280 18 292 26 Q300 16 314 22 Q328 14 338 26 Q350 20 354 34 Q366 38 364 52 Q372 62 364 72 Q368 84 356 90 Q354 102 340 104 Q332 114 318 108 Q306 118 292 110 Q278 118 268 106 Q254 102 254 90 Q242 84 244 70 Q236 58 248 50 Q256 42 270 50Z" fill="none" stroke="#5dcaa5" strokeWidth="1"/>

    {/* Pill capsule center-right */}
    <ellipse cx="390" cy="300" rx="30" ry="13" fill="none" stroke="#5dcaa5" strokeWidth="1.1" transform="rotate(-25 390 300)"/>
    <line x1="368" y1="287" x2="412" y2="313" stroke="#5dcaa5" strokeWidth="0.9"/>
    ...
    {/* AI neural nodes — center-right */}
    {[[360,480],[360,530],[360,580],[430,505],[430,555]].map(([x,y],i)=>(
      <circle key={i} cx={x} cy={y} r="5" fill="none" stroke="#5dcaa5" strokeWidth="0.9"/>
    ))}
    {[[360,480,430,505],[360,480,430,555],[360,530,430,505],[360,530,430,555],[360,580,430,555]].map(([x1,y1,x2,y2],i)=>(
      <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#5dcaa5" strokeWidth="0.6"/>
    ))}
  </svg>
)
...
export default function RadiologistRegister() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch(`/api/admin/radiologists/validate-token?token=${token}`)
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (res.data.message==='User registered successfully'||res.data.message==='Registration completed successfully') {
        ...
        if (inviteToken) {
          try {
            await api.post('/admin/radiologists/link-invite', { token: inviteToken, email })
            ...
            <form onSubmit={handleRegister}>
              ...
              <div style={{display:'flex',gap:10,marginBottom:12}}>
                <div style={{flex:1}}>
                  ...
                  <input style={inp({background: inviteToken ? '#e8ecef' : T.offWhite, color: inviteToken ? '#6b7280' : T.textDark})} placeholder="First name" value={firstName} onChange={e=>setFirstName(e.target.value)} required={!googleUser} disabled={!!inviteToken}/>
                </div>
                <div style={{flex:1}}>
                  <label style={lbl}>Last Name</label>
                  <input style={inp({background: inviteToken ? '#e8ecef' : T.offWhite, color: inviteToken ? '#6b7280' : T.textDark})} placeholder="Last name" value={lastName} onChange={e=>setLastName(e.target.value)} disabled={!!inviteToken}/>
                </div>
              </div>
