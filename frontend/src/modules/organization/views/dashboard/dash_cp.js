...
const GLOBAL_CSS = `
  ...
  .radiology-app.body-scan-open .sidebar,
  .radiology-app.body-scan-open .header {
    display: none !important;
  }
  .radiology-app.body-scan-open .wrapper {
...
const tatData = [
  { label: "STAT / Emergency", target: "Target ≤ 1 hr", val: "0.8h", trend: "↓ 0.1", ok: "good" },
  { label: "Critical / Urgent", target: "Target ≤ 4 hr", val: "3.2h", trend: "↓ 0.4", ok: "good" },
  { label: "Routine", target: "Target ≤ 24 hr", val: "19.4h", trend: "↑ 1.8", ok: "warn" },
  { label: "MRI Brain (complex)", target: "Target ≤ 8 hr", val: "11.2h", trend: "↑ 2.3", ok: "bad" },
  { label: "CT Oncology", target: "Target ≤ 12 hr", val: "9.7h", trend: "→", ok: "good" },
  { label: "Average overall", target: "All modalities", val: "7.6h", trend: "↑ 0.6", ok: "warn" },
];
...
const qcData = [
  { label: "Metadata QC",     value: 4, color: "#f87171", bg: "rgba(239,68,68,0.12)", bar: 20 },
  { label: "Image QC", value: 7, color: "#fbbf24", bg: "rgba(245,158,11,0.12)", bar: 35 },
  { label: "Open queries",         value: 11, color: "#fb923c", bg: "rgba(251,146,60,0.12)", bar: 55 },
];
...
const WL_FILTERS = ["All", "Overdue", "STAT", "Unassigned"];
...
export default function Dashboard() {
  ...
  const [bodyCardView, setBodyCardView] = useState("values");
  ...
  const handleDownloadCase = (caseItem) => {
    ...
    setTimeout(() => {
      const reportText =
        ...
        `All images, profiles, and data are fictional and for demonstration only.\n\n` +
        `IMPRESSION:\n` +
        `1. No acute abnormality detected (demo data).\n` +
      ...
      const imageNote =
        `SCAN IMAGES NOTE\n` +
        `================\n` +
        `Case: ${caseItem.caseId} | Modality: ${caseItem.modality}\n` +
        `This placeholder represents the ${caseItem.modality} scan images.\n` +
        `In production, DICOM files for this study would be included here.\n`;

      const zipContent = `${reportText}\n---\n${imageNote}`;
      const blob = new Blob([zipContent], { type: "application/octet-stream" });
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement("a");
      anchor.href = url;
      anchor.download = `${caseItem.caseId}_report_and_images.zip`;
  ...
  useEffect(() => {
    const appRoot = document.querySelector(".radiology-app");
    if (!appRoot) return;

    appRoot.classList.toggle("body-scan-open", bodyOverviewOpen);

    return () => {
      appRoot.classList.remove("body-scan-open");
    };
  }, [bodyOverviewOpen]);

  const filteredWL = WORKLIST.filter((r) => {
    if (wlFilter === "All") return true;
    if (wlFilter === "Overdue") return r.status === "OVERDUE";
    if (wlFilter === "STAT") return r.priority === "STAT";
    if (wlFilter === "Unassigned") return r.assignee === "Unassigned";
    return true;
  });
    ...
    <>
      ...
      >
          ...
          {kpis.map((k, i) => (
            <div
              ...
              onClick={() => {
                ...
                else if (k.label === "Routine Queue") setRoutineQueueModal(true);
        ...
        >
          ...
          >
            <Title color={titleClr}>TAT Performance · Hours</Title>
            ...
            {tatData.map((t, i) => (
              ...
              >
                <div>
                  <div
                    style={{ fontSize: 15, color: textPri, fontWeight: 600 }}
                  >
                    {t.label}
                  </div>
                  <div
                    style={{ fontSize: 15, color: textMuted, marginTop: 3 }}
                  >
                    {t.target}
                  </div>
                </div>
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  <div
                    style={{ fontSize: 15, color: textMuted, marginTop: 2 }}
                  >
                    {t.trend}
                  </div>
                </div>
          ...
          <div className="dash-card" style={{ background:cardBg, borderRadius:18, padding:22, boxShadow:cardShadow, border:`1px solid ${cardBorder}`, animationDelay:"260ms" }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:16, gap:8 }}>
              <div>
                <div style={{ ...sg, fontSize:14, fontWeight:700,textTransform:"uppercase", color:textPri }}>Quality Check</div>
                <div style={{ ...sg, fontSize:12, letterSpacing:"1.8px", color:isDark?"rgba(255,255,255,0.4)":"rgba(0,0,0,0.4)",  fontWeight:700, marginTop:3 }}>Validation</div>
              </div>
              ...
              {qcData.filter(q=> qcCardMode==="upload" || q.label!=="Image QC").map((q)=>{
                ...
                return (
                  <div key={q.label}
                    ...
                    <div>
                      <div style={{ fontSize:14, color:textPri, fontWeight:600 }}>{q.label}</div>
                    ...
                    <div style={{ ...mono, fontSize:28, fontWeight:700, color:q.color, flexShrink:0, marginLeft:14 }}>{displayValue}</div>
                  </div>
                );
              })}
            </div>
          ...
          {qcModal && (()=>{
            ...
            // helpers
            const ModBadge = ({mod}) => { const [bg,fg]=QC_MOD_C[mod]||["#333","#ccc"]; return <span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:99,background:bg,color:fg,whiteSpace:"nowrap"}}>{mod}</span>; };
            const ChkBadge = ({chk}) => { const c=QC_CHK_C[chk]||{bg:"#333",fg:"#ccc"}; return <span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:99,background:c.bg,color:c.fg,whiteSpace:"nowrap"}}>{chk}</span>; };
            const StatusPill = ({s}) => {
              ...
              return <span style={{fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:99,background:bg,color:fg,whiteSpace:"nowrap"}}>{s}</span>;
            };
            const TH = ({ch}) => <th style={{padding:"7px 10px",textAlign:"left",fontSize:10,fontWeight:700,color:tS,textTransform:"uppercase",letterSpacing:".05em",background:isDark?"rgba(255,255,255,0.05)":"#f1f5f9",borderBottom:`1px solid ${mBdr}`,whiteSpace:"nowrap"}}>{ch}</th>;
            const TD = ({ch,clr,mono:m2}) => <td style={{padding:"8px 10px",fontSize:11,color:clr||tP,borderBottom:`1px solid ${rBdr}`,verticalAlign:"middle",fontFamily:m2?"'DM Mono',monospace":"inherit"}}>{ch}</td>;
            ...
            const ThreshBlock = ({groups}) => (
              <div style={{marginTop:12}}>
                {groups.map(g=>{
                  ...
                  return (
                    <div key={g.check} style={{marginBottom:12}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                        <span style={{fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,background:cc.bg,color:cc.fg}}>{g.check}</span>
                        <span style={{fontSize:11,color:tS}}>thresholds</span>
                      </div>
                      <div style={{border:`1px solid ${mBdr}`,borderRadius:8,overflow:"hidden"}}>
                        <div style={{display:"grid",gridTemplateColumns:"1.3fr 1fr 1fr",background:isDark?"rgba(255,255,255,0.05)":"#f9fafb",padding:"6px 10px",borderBottom:`1px solid ${mBdr}`}}>
                          <div style={{fontSize:10,fontWeight:700,color:tS,textTransform:"uppercase",letterSpacing:".05em"}}>Field</div>
                          <div style={{fontSize:10,fontWeight:700,color:"#34d399",textTransform:"uppercase",letterSpacing:".05em"}}>Pass ✓</div>
                          <div style={{fontSize:10,fontWeight:700,color:"#f87171",textTransform:"uppercase",letterSpacing:".05em"}}>Fail ✗</div>
                        </div>
                        {g.items.map((r,i)=>(
                          <div key={i} style={{display:"grid",gridTemplateColumns:"1.3fr 1fr 1fr",padding:"7px 10px",background:i%2===0?rBg:mBg,borderBottom:i<g.items.length-1?`1px solid ${rBdr}`:"none"}}>
                            <div style={{fontSize:11,fontWeight:600,color:tP}}>{r.field}</div>
                            <div style={{fontSize:11,color:"#34d399"}}>{r.pass}</div>
                            <div style={{fontSize:11,color:"#f87171"}}>{r.fail}</div>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            return (
              <div onClick={()=>setQcModal(null)} style={{position:"fixed",inset:0,zIndex:99999,background:"rgba(0,0,0,0.60)",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"80px 20px 20px"}}>
                <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:860,background:mBg,border:`1px solid ${mBdr}`,borderRadius:20,overflow:"hidden",boxShadow:"0 28px 70px rgba(0,0,0,0.4)",maxHeight:"calc(100vh - 100px)",display:"flex",flexDirection:"column"}}>

                  {/* Modal header — single compact row */}
                  <div style={{padding:"6px 16px",borderBottom:`1px solid ${mBdr}`,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                    <span style={{fontSize:12,fontWeight:700,color:tP,flex:1,whiteSpace:"nowrap"}}>Quality Check</span>
                    ...
                    <button onClick={()=>setQcModal(null)} style={{background:"transparent",border:"none",color:tS,width:22,height:22,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,lineHeight:1,borderRadius:4}}>×</button>
                  </div>
                  {/* 4 Tabs */}
                  <div style={{display:"flex",gap:0,borderBottom:`1px solid ${mBdr}`,flexShrink:0,overflowX:"auto"}}>
                    {TABS.map(t=>(
                      <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",border:"none",borderBottom: tab===t.id?`2.5px solid ${t.color}`:"2.5px solid transparent",background:"transparent",color: tab===t.id?t.color:tS,transition:"color 0.15s",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6}}>
                        {t.label}
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:99,background:isDark?`${t.color}22`:t.color+"18",color:t.color,fontWeight:700}}>{t.count}</span>
                      </button>
                    ))}
                  </div>

                  {/* Body */}
                  <div className="modal-scroll" style={{overflowY:"auto",padding:"12px 18px 20px",flex:1}}>
                    ...
                    {isUpload && tab==="protocol" && (()=> {
                      ...
                      return (
                      <div>
                        <div style={{padding:"9px 13px",background:isDark?"rgba(248,113,113,0.1)":"#FEF2F2",border:"1px solid #fca5a5",borderRadius:8,fontSize:11,color:"#f87171",marginBottom:14}}>
                        ...
                        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:tS,marginBottom:8}}>Cases — rejection reason &amp; what to change</div>
                        <div style={{border:`1px solid ${mBdr}`,borderRadius:10,overflow:"hidden",marginBottom:16}}>
                          <table style={{width:"100%",borderCollapse:"collapse"}}>
                            ...
                            <tbody>{rows.length === 0 ? (
                              <tr><td colSpan={8} style={{textAlign:"center",padding:"24px 20px",color:tS,fontSize:12}}>No Metadata QC found — all uploaded cases passed ID &amp; Slice checks.</td></tr>
                            ) : rows.map((r,i)=>(
                              <tr key={r.id} style={{background:i%2===0?rBg:mBg}}>
                                <TD ch={<code style={{fontSize:9,background:isDark?"rgba(255,255,255,0.08)":"#F3F4F6",padding:"1px 5px",borderRadius:3,color:tP}}>{r.patientId}</code>}/>
                        ...
                        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:tS,marginBottom:4}}>Check thresholds for Metadata QC</div>
                    ...
                    {isUpload && tab==="quality" && (()=> {
                      ...
                      return (
                      <div>
                        <div style={{padding:"9px 13px",background:isDark?"rgba(251,191,36,0.1)":"#FFFBEB",border:"1px solid #fcd34d",borderRadius:8,fontSize:11,color:"#fbbf24",marginBottom:14}}>
                        ...
                        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:tS,marginBottom:8}}>Cases — rejection reason, measured value &amp; threshold</div>
                        <div style={{border:`1px solid ${mBdr}`,borderRadius:10,overflow:"hidden",marginBottom:16}}>
                          <table style={{width:"100%",borderCollapse:"collapse"}}>
                            <thead><tr>
                            ...
                            <tbody>{rows.length === 0 ? (
                              <tr><td colSpan={10} style={{textAlign:"center",padding:"24px 20px",color:tS,fontSize:12}}>No Image QC found — all uploaded files passed Content &amp; Pixel checks.</td></tr>
                            ) : rows.map((r,i)=>(
                              <tr key={r.id} style={{background:i%2===0?rBg:mBg}}>
                                <TD ch={<code style={{fontSize:9,background:isDark?"rgba(255,255,255,0.08)":"#F3F4F6",padding:"1px 5px",borderRadius:3,color:tP}}>{r.patientId}</code>}/>
                                <TD ch={<span style={{fontWeight:600}}>{r.name}</span>}/>
                                ...
                                <TD ch={r.file} clr={tS}/>
                                <TD ch={<ChkBadge chk={r.check}/>}/>
                                <TD ch={<span style={{color:"#f87171",fontSize:11}}>{r.reason}</span>}/>
                                <TD ch={<span style={{...{fontFamily:"'DM Mono',monospace"},fontSize:10,color:"#fb923c"}}>{r.failValue}</span>}/>
                                <TD ch={<span style={{...{fontFamily:"'DM Mono',monospace"},fontSize:10,color:"#34d399"}}>{r.threshold}</span>}/>
                        ...
                        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:tS,marginBottom:4}}>Check thresholds for Image QC</div>
                    ...
                    {isUpload && tab==="open" && (()=> {
                      ...
                      return (
                      <div>
                        <div style={{padding:"9px 13px",background:isDark?"rgba(251,146,60,0.1)":"#FFF7ED",border:"1px solid #fdba74",borderRadius:8,fontSize:11,color:"#fb923c",marginBottom:14}}>
                        ...
                        {unresolved.length === 0 ? (
                          <div style={{padding:"24px 20px",textAlign:"center",color:tS,fontSize:12,background:isDark?"rgba(255,255,255,0.03)":"#f8fafc",border:`1px solid ${mBdr}`,borderRadius:10,marginBottom:16}}>
                            No open queries — nothing is currently waiting for admin or organisation review.
                          </div>
                        ) : unresolved.map(q=>{
                          const cc=QC_CHK_C[q.check]||{bg:"#333",fg:"#ccc"};
                          return (
                            <div key={q.id} style={{border:"1px solid #fb923c",borderRadius:10,marginBottom:8,overflow:"hidden"}}>
                              <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"11px 14px",background:isDark?"rgba(251,146,60,0.07)":"#FFF7ED"}}>
                                <span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:99,background:cc.bg,color:cc.fg,flexShrink:0,marginTop:2}}>{q.check}</span>
                                <div style={{flex:1}}>
                                  <div style={{fontWeight:700,fontSize:12,color:tP}}>{q.name} <span style={{color:tS,fontWeight:400,fontSize:11}}>· {q.patientId}</span> <ModBadge mod={q.modality}/></div>
                                  <div style={{fontSize:11,color:"#fb923c",margin:"4px 0"}}>{q.query}</div>
                                  <div style={{fontSize:10,color:tS}}>Assigned to: <strong style={{color:tP}}>{q.org}</strong> · Opened {q.opened} · Action: <em>{q.action}</em></div>
                                </div>
                                <span style={{fontSize:10,padding:"2px 9px",borderRadius:99,background:q.daysOpen===0?"#EAF3DE":"#FAEEDA",color:q.daysOpen===0?"#27500A":"#633806",fontWeight:700,flexShrink:0}}>{q.daysOpen===0?"Today":`${q.daysOpen}d open`}</span>
                              </div>
                            </div>
                          );
                        })}
                        ...
                        {resolved.length > 0 && (
                          <>
                            <div style={{display:"flex",alignItems:"center",gap:8,margin:"20px 0 10px 0"}}>
                              <div style={{flex:1,height:1,background:mBdr}}/>
                              <span style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".1em",color:"#34d399"}}>
                            ...
                            {resolved.map(r=>{
                              ...
                              return (
                                <div key={r.id} style={{border:"1px solid #34d399",borderRadius:10,marginBottom:8,overflow:"hidden"}}>
                                  <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"11px 14px",background:isDark?"rgba(52,211,153,0.07)":"#F0FDF4"}}>
                                    <span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:99,background:cc.bg,color:cc.fg,flexShrink:0,marginTop:2}}>{r.check}</span>
                                    <div style={{flex:1}}>
                                      <div style={{fontWeight:700,fontSize:12,color:tP}}>{r.name} <span style={{color:tS,fontWeight:400,fontSize:11}}>· {r.patientId}</span> <ModBadge mod={r.modality}/></div>
                                      <div style={{fontSize:11,color:"#f87171",margin:"3px 0"}}>Was: {r.was}</div>
                                      <div style={{fontSize:11,color:"#34d399"}}>Fix: {r.fix}</div>
                                      <div style={{fontSize:10,color:tS,marginTop:3}}>Resolved by <strong style={{color:tP}}>{r.by}</strong> at {r.time}</div>
                                    </div>
                                    <span style={{fontSize:10,padding:"2px 9px",borderRadius:99,background:"#EAF3DE",color:"#27500A",fontWeight:700,flexShrink:0}}>✓ Resolved</span>
                                  </div>
                                </div>
                              );
                            })}
                    ...
                    {!isUpload && tab==="protocol" && (
                      <div>
                        ...
                        {QC_REPORT_DATA.protocol.map(rep=>{
                          ...
                          return (
                            <div key={rep.id} style={{border:`1px solid ${ovBd}`,borderRadius:10,marginBottom:12,overflow:"hidden"}}>
                              {/* Report header */}
                              <div style={{padding:"9px 14px",background:isDark?"rgba(255,255,255,0.04)":"#F9FAFB",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",borderBottom:`1px solid ${ovBd}`}}>
                                <span style={{fontWeight:700,fontSize:12,color:tP}}>{rep.name}</span>
                                <code style={{fontSize:9,background:isDark?"rgba(255,255,255,0.08)":"#F3F4F6",padding:"1px 5px",borderRadius:3,color:tS}}>{rep.patientId}</code>
                                <ModBadge mod={rep.modality}/>
                                <span style={{fontSize:10,color:tS}}>· {rep.rad} · {rep.date}</span>
                                <span style={{marginLeft:"auto",fontSize:10,fontWeight:700,padding:"2px 9px",borderRadius:99,background:ovBg,color:ovFg}}>
                              ...
                              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}}>
                                {SECTIONS.map((sec,si)=>(
                                  <div key={sec.key} style={{padding:"8px 12px",borderRight:si%2===0?`1px solid ${mBdr}`:"none",borderBottom:si<2?`1px solid ${mBdr}`:"none"}}>
                                    <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:6}}>
                                      <span style={{fontSize:9,fontWeight:700,padding:"1px 6px",borderRadius:4,background:sec.bg,color:sec.fg}}>{sec.label}</span>
                                    </div>
                                    {rep.checks[sec.key].map((it,ii)=>{
                                      ...
                                      return (
                                        <div key={ii} style={{display:"flex",alignItems:"flex-start",gap:5,marginBottom:4}}>
                                          <div style={{width:6,height:6,borderRadius:"50%",background:dotC,flexShrink:0,marginTop:4}}/>
                                          <div style={{flex:1}}>
                                            <span style={{fontSize:10,fontWeight:600,color:tP}}>{it.field} </span>
                                            <span style={{fontSize:10,color:valC}}>— {it.value}</span>
                                          </div>
                                        </div>
                                      );
                                    })}
                    ...
                    {!isUpload && tab==="open" && (
                      <div>
                        ...
                        {QC_REPORT_DATA.open.map((r,i)=>{
                          ...
                          return (
                            <div key={r.id} style={{border:`1px solid ${sv.bd}`,borderRadius:10,marginBottom:10,overflow:"hidden"}}>
                              <div style={{padding:"10px 14px",background:isDark?`${sv.bd}18`:`${sv.bg}66`,display:"flex",alignItems:"flex-start",gap:10}}>
                                <span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99,background:sv.bg,color:sv.fg,flexShrink:0,marginTop:2}}>{sv.icon} {r.severity==="error"?"Error":"Warning"}</span>
                                <div style={{flex:1}}>
                                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4,flexWrap:"wrap"}}>
                                    <span style={{fontWeight:700,fontSize:12,color:tP}}>{r.name}</span>
                                    <code style={{fontSize:9,background:isDark?"rgba(255,255,255,0.08)":"#F3F4F6",padding:"1px 5px",borderRadius:3,color:tS}}>{r.patientId}</code>
                                    <ModBadge mod={r.modality}/>
                                    <span style={{fontSize:9,color:tS}}>· {r.rad}</span>
                                    <span style={{marginLeft:"auto",fontSize:9,fontWeight:700,color:r.days===0?"#34d399":r.days<=1?"#fbbf24":"#f87171"}}>{r.days===0?"Today":`${r.days}d open`}</span>
                                  </div>
                                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                                    <span style={{fontSize:9,fontWeight:700,padding:"1px 7px",borderRadius:4,background:secBg,color:secFg}}>{r.section}</span>
                                    <span style={{fontSize:10,fontWeight:600,color:tP}}>→ {r.field}</span>
                                  </div>
                                  <div style={{fontSize:11,color:sv.fg,marginBottom:4}}>{r.issue}</div>
                                  <div style={{fontSize:10,color:tS}}>Action: <span style={{color:tP,fontWeight:600}}>{r.action}</span> · Assigned to: <span style={{color:tP,fontWeight:600}}>{r.assigned}</span></div>
                                </div>
                              </div>
                            </div>
                          );
                            ...
                            {QC_REPORT_DATA.resolved.map((r,i)=>{
                              ...
                              return (
                                <div key={r.id} style={{border:"1px solid #34d399",borderRadius:10,marginBottom:10,overflow:"hidden"}}>
                                  <div style={{padding:"10px 14px",background:isDark?"rgba(52,211,153,0.07)":"#F0FDF4",display:"flex",alignItems:"flex-start",gap:10}}>
                                    <span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99,background:"#EAF3DE",color:"#27500A",flexShrink:0,marginTop:2}}>✓ Resolved</span>
                                    <div style={{flex:1}}>
                                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4,flexWrap:"wrap"}}>
                                        <span style={{fontWeight:700,fontSize:12,color:tP}}>{r.name}</span>
                                        <code style={{fontSize:9,background:isDark?"rgba(255,255,255,0.08)":"#F3F4F6",padding:"1px 5px",borderRadius:3,color:tS}}>{r.patientId}</code>
                                        <ModBadge mod={r.modality}/>
                                        <span style={{fontSize:9,color:tS}}>· {r.rad}</span>
                                        <span style={{marginLeft:"auto",fontSize:9,fontWeight:700,color:"#34d399"}}>{r.time}</span>
                                      </div>
                                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                                        <span style={{fontSize:9,fontWeight:700,padding:"1px 7px",borderRadius:4,background:secBg,color:secFg}}>{r.section}</span>
                                        <span style={{fontSize:10,fontWeight:600,color:tP}}>→ {r.field}</span>
                                      </div>
                                      <div style={{fontSize:11,color:"#f87171",marginBottom:5}}>Was: {r.was}</div>
                                      <div style={{fontSize:11,color:"#34d399",marginBottom:3}}>Fix: ✓ {r.fix}</div>
                                      <div style={{fontSize:10,color:tS}}>Resolved by <span style={{color:tP,fontWeight:600}}>{r.by}</span></div>
                                    </div>
                                  </div>
                                </div>
                              );
              ...
              >
                ...
                >
                  {[
                    ...
                    { key: "body", label: "Interactive View" },
                  ].map((item) => {
                    const active = bodyCardView === item.key;
                    return (
                      <button
                        key={item.key}
                        type="button"
                        onClick={() => setBodyCardView(item.key)}
                        style={{
                          border: "none",
                          background: active ? "#2563eb" : "transparent",
                          color: active ? "#ffffff" : (isDark ? "#cbd5e1" : "#2563eb"),
                          padding: "8px 14px",
                          borderRadius: 10,
                          fontSize: 13,
                          fontWeight: 700,
            ...
            <div>
              {bodyCardView === "values" ? (
                ...
                >
                  {modalityValueData.map((item, index) => (
                    <div
                      ...
                      style={{
                        display: "grid",
                        gridTemplateColumns: "42px 1fr 44px 48px",
                        alignItems: "center",
                        gap: 10,
                        padding: "12px 0",
                        borderBottom:
                ...
                <BodyScanOverview
                  compact
                  bodyOnly
                  onOpenFullscreen={() => setBodyOverviewOpen(true)}
                />
              )}
            </div>
          </div>
              ...
              {WL_FILTERS.map((f) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    border:
                      wlFilter === f ? "none" : "1px solid #cbd5e1",
                    color: wlFilter === f ? "#fff" : "#64748b",
                    background:
                      wlFilter === f
                        ? "linear-gradient(135deg,#1e3a8a,#2563eb)"
                        : "#fff",
          ...
          >
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>↑ 8.4% vs target · 26 Mar 2026</div>
              </div>
              {/* Stats badges */}
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                {[
                  { label: "STAT",    value: COMPLETED_CASES.filter(c => c.priority === "STAT").length,    color: "#f87171" },
                  { label: "Urgent",  value: COMPLETED_CASES.filter(c => c.priority === "Urgent").length,  color: "#fbbf24" },
                  { label: "Routine", value: COMPLETED_CASES.filter(c => c.priority === "Routine").length, color: "#34d399" },
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12, color: textMuted }}>
                Showing {COMPLETED_CASES.length} of 312 completed cases · 26 Mar 2026
              </span>
              <span style={{ fontSize: 12, color: textMuted }}>
          ...
          >
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>41 urgent · 6 STAT · 26 Mar 2026</div>
              </div>
              {/* Summary badges */}
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                {[
                  { label: "Assigned",   value: ROUTINE_QUEUE_CASES.filter(c => c.status === "Assigned").length,   ...STATUS_META.Assigned },
                  { label: "Unassigned", value: ROUTINE_QUEUE_CASES.filter(c => c.status === "Unassigned").length, ...STATUS_META.Unassigned },
                  { label: "Rejected",   value: ROUTINE_QUEUE_CASES.filter(c => c.status === "Rejected").length,   ...STATUS_META.Rejected },
                ].map(s => (
                  <div key={s.label} style={{
                    background: "rgba(255,255,255,0.12)", borderRadius: 10,
                    padding: "8px 14px", textAlign: "center",
                      ...
                      <tr key={c.caseId} style={{
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{
                            ...
                            {c.status === "Assigned" ? `${c.status} · ${c.assignedTo}` : c.status}
                          </span>
                        </td>
                      </tr>
                    );
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12, color: textMuted }}>
                Showing {ROUTINE_QUEUE_CASES.length} of 200 queued cases · 26 Mar 2026
              </span>
              <span style={{ fontSize: 12, color: textMuted }}>
                Unassigned cases require radiologist assignment before reading
              </span>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>SLA breach: 17 · Critical: 6 · 26 Mar 2026</div>
              </div>
              {/* Summary badges */}
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                {[
            ...
            <div style={{
              ...
              <span style={{ fontSize: 12, color: textMuted }}>
                Showing {PENDING_OVERDUE_CASES.length} of 247 pending / overdue cases · 26 Mar 2026
              </span>
              <span style={{ fontSize: 12, color: textMuted }}>
