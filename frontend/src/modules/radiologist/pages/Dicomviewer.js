  ...
  import useViewerDataLoader, { toSameOriginUrl } from "./DicomViewer/hooks/useViewerDataLoader";
  ...
  const SingleSliceStrip = React.memo(function SingleSliceStrip({ current, total, onStep }) {
    ...
    // Dragging is RELATIVE (fixed pixels-per-slice), not absolute position-on-
    // track — with hundreds of slices mapped onto a short track, a proportional
    // seek made a millimeter of mouse movement skip dozens of slices at once.
    // Only the initial click still jumps to that position (standard scrubber
    // behavior); the drag that follows moves at a controlled, constant rate
    // regardless of how many slices the series has.
    const dragRef = React.useRef(null); // { lastY, carryPx }
    const DRAG_PX_PER_SLICE = 6;
    ...
    return (
      <div ref={trackRef}
        ...
        onPointerDown={(e) => {
          ...
          seekTo(e.clientY, current);
          dragRef.current = { lastY: e.clientY, carryPx: 0 };
        }}
        onPointerMove={(e) => {
          ...
          const carryPx = dragRef.current.carryPx + dy;
          const steps = Math.trunc(carryPx / DRAG_PX_PER_SLICE);
          dragRef.current.carryPx = carryPx - steps * DRAG_PX_PER_SLICE;
  ...
  const {
    ...
    // Reset to single-pane whenever the viewer opens a case (fresh mount or
    // switching cases without a full remount). Keyed on caseId as well as
    // fileUrl/filename so a case switch always resets even if those happen
    // to be identical (e.g. both still loading) — otherwise a grid layout
    // left selected on the previous case leaks into the next one and every
    // cell shows the same not-yet-reassigned series until the grid is
    // reselected.
    useEffect(() => {
      ...
      setCompareViewportSeries({ 0: null, 1: null, 2: null, 3: null });
      setFocusedCompareSlot(0);
    }, [fileUrl, filename, caseId]);
    ...
    useVolumeMprCrosshair({
      // Exact same rule as the volMpr call above, but scoped to the stack
      // grids (mpr3 / compare2x2 / multi-grid) this call is actually for.
      // volMpr is excluded because the call above already owns it; volMip
      // is excluded too — it's a single flat projection pane with nothing
      // to cross-reference, so a 3D Pointer crosshair there is meaningless
      // (and getVisibleDicomSlots() only returns 1 slot for volMip, so this
      // hook was tripling the same pane into its 3-slot crosshair anyway).
      enabled: layoutMode !== "volMpr" && layoutMode !== "volMip" && !isNifti && (crosshairEnabled || syncEnabled),
      mode: crosshairMode,
      renderingEngineRef,
    ...
    useEffect(() => {
      ...
      const overrideSlot = async (slot) => {
        ...
        /* PERF: rewrite presigned S3 urls to same-origin streaming urls (pure
          string work, no I/O) instead of eagerly fetching + blob-ifying every
          slice up front — that was downloading the entire series (up to
          150MB+) before a single pixel could paint, which is what made
          switching series feel sluggish. */
        const imageIds = series.urls.map(toSameOriginUrl).map(buildImageId);
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        /* PERF: same-origin rewrite instead of eagerly fetching + blob-ifying
          every slice up front (see useViewerDataLoader's toSameOriginUrl —
          this used to download the whole series before the first pixel
          painted, which is what made series switches feel sluggish). */
        const imageIds = series.urls.map(toSameOriginUrl).map(buildImageId);
        const midIdx = Math.floor(imageIds.length / 2);
        console.log(
    ...
    useEffect(() => {
      ...
      /* dicomTotalSlices intentionally excluded: the single-override/mpr-override
        effects already swap the mounted stack in place when the user switches
        series, and rebuildCornerstoneDicomViewports tears down + recreates the
        viewport from a THINNED preview stack (previewStep logic) — including
        it here meant every series switch briefly rebuilt on a truncated stack
        and flashed a wrong (smaller) slice count before the real one settled. */
    }, [isCornerstoneDicom, dicomGrid, dicomSlotPlanes, showReport, showSidebar, isFullscreen, reportViewerCollapsed]);
    ...
    /* ─── Keyboard slice scroll (single viewport) ───────────────────
      ...
      uses, so the "Slices: X/Y" overlay stays in sync either way. Ignored
      while typing in an input/textarea/contentEditable (report editor,
      chat, annotation text) so arrow keys keep their normal text-editing
      behavior there. */
    useEffect(() => {
      if (!isCornerstoneDicom || showDicomTriPlanar) return;
      const onKeyDown = (e) => {
        ...
        e.preventDefault();
        scrollCornerstoneDicomBySlot(0, e.key === "ArrowDown" ? 1 : -1);
      };
      window.addEventListener("keydown", onKeyDown);
      return () => window.removeEventListener("keydown", onKeyDown);
    }, [isCornerstoneDicom, showDicomTriPlanar, scrollCornerstoneDicomBySlot]);
    ...
    return (
      <div ref={viewerRootRef} style={{ height: "100vh", width: "100%", background: "#000", overflow: "hidden" }}>
  ...
  clientId={clientId}
          ...
          <CCardBody className="p-0 position-relative flex-grow-1" style={{ backgroundColor: "black", overflow: "hidden", minHeight: 0 }}>
            {error && (
              ...
              >
                <div style={{
                  ...
                  <DicomToolbar
                    ...
                    showMetadata={showDicomMetadata}
                    onToggleMetadata={() => setShowDicomMetadata((v) => !v)}
                    /* ─ Slab projection: MIP / MinIP / Average ─ */
                  ...
                  <div
                    ...
                    {showDicomTriPlanar ? (
                      ...
                      <div
                        ...
                        /* Wheel scroll is driven natively by Cornerstone's StackScrollTool
                          (bound in useViewerDataLoader), so it moves the image directly
                          without touching React state. Resync the slice counter after the
                          native handler has applied the scroll — otherwise the "Slices: X/Y"
                          overlay only updates on the prev/next buttons, which explicitly
                          call refreshDicomSliceIndicators themselves. */
                        onWheel={() => requestAnimationFrame(() => refreshDicomSliceIndicators([0]))}
                      />
                    )}

                    {/* Single-viewport: top-left patient identity */}
                    {!showDicomTriPlanar && showDicomMetadata && (() => {
                    ...
                    {!showDicomTriPlanar && (() => {
                      ...
                      // Single-viewport mode has exactly one viewport ("DICOM_SINGLE"),
                      // always tracked under slice-indicator key 0 — series switches and
                      // wheel-scroll sync write to key 0, so reads here must match that
                      // key rather than activeDicomSlot (which can be stale from a
                      // previous grid/main2 session and silently point at the wrong entry).
                      const _step = (delta) => scrollCornerstoneDicomBySlot(0, delta);
                      ...
                      return (
                        <div style={{ position: "absolute", top: 6, right: 6, zIndex: 40, display: "flex", flexDirection: "column", gap: 3, alignItems: "flex-end" }}>
                          ...
                          {showDicomMetadata && anatomyText && <div style={{ ...infoStyle, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{anatomyText}</div>}
                        </div>
                      );
                    })()}
                    ...
                    {!showDicomTriPlanar && showDicomMetadata && (() => {
                      ...
                      return (
                        <div style={{ position: "absolute", left: 8, bottom: 6, zIndex: 30, display: "flex", flexDirection: "column", pointerEvents: "none" }}>
                          ...
                          <span style={{ ..._mono, color: "#cbd5e1", marginTop: 2 }}>{`Slices: ${dicomSliceBySlot[0]?.current ?? 1}/${dicomSliceBySlot[0]?.total ?? 1}`}</span>
                          <span style={{ ..._mono, color: "#cbd5e1" }}>{`X:${dicomMouseBySlot[0]?.x ?? "-"} Y:${dicomMouseBySlot[0]?.y ?? "-"}`}</span>
                        </div>
                      );
                    })()}
                    ...
                    {!showDicomTriPlanar && (
                      <SingleSliceStrip
                        current={dicomSliceBySlot[0]?.current ?? 1}
                        total={dicomSliceBySlot[0]?.total ?? 1}
                        onStep={(delta) => scrollCornerstoneDicomBySlot(0, delta)}
                      />
                    )}
