    ...
    fetch(`${BACKEND_URL}/radiology/reports/${encodeURIComponent(caseId)}?user_id=${encodeURIComponent(uid)}`)
      ...
      .then((json) => {
        if (json?.success && json.data) {
          const d = json.data;
          const hasAnySection = d.technique || d.findings || d.impression || d.opinions;
          setReportData((prev) => ({ ...(prev || {}), ...d }));
          setReportExists(!!hasAnySection);
        }
      })
      .catch(() => {});
  }, [caseId]);
...
clientId={clientId}
          onToggleReport={async () => {
            // Preserve AI content + fill metadata
            setReportData((prev) => ({
            ...
            // Trigger slicing animation (fake if saved, real if not)
            if (typeof generateAIReport === "function") {
              generateAIReport({ skipUserMessage: true });
            } else {
              setShowReport(true);
            }
          }}
                      ...
                      if (medsamMask) {
                        ...
                        try {
                          ...
                          const res = await fetch("/api/radiology/ai/analyze", {
                            ...
                            body: JSON.stringify({ image_base64: b64, prompt, model: aiModel || "llava" }),
                          });
