...
export default function useCornerstoneDicomControls({
  ...
  const scrollCornerstoneDicomBySlot = (slot, delta) => {
    ...
    const engine = renderingEngineRef.current;
    // In single-viewport mode the canvas is registered as "DICOM_SINGLE",
    // not "DICOM_SLOT_N", so fall back to that ID when the slot lookup misses.
    const vp = engine?.getViewport?.(vpId) ?? engine?.getViewport?.("DICOM_SINGLE");
    if (!vp?.scroll) return;
    vp.scroll(delta);
    vp.render();
