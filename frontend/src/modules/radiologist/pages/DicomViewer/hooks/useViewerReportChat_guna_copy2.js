...
export default function useViewerReportChat({
  ...
  const editReportWithCommand = async (command) => {
    ...
    const blocks = Array.from(root.querySelectorAll(".report-answer.block"));
    const current = {
      technique:  safeText(blocks[0]?.innerText) || "",
      findings:   safeText(blocks[1]?.innerText) || "",
      impression: safeText(blocks[2]?.innerText) || "",
      opinions:   safeText(blocks[3]?.innerText) || "",
    };
    ...
    if (blocks[0]) blocks[0].innerText = updated.technique  ?? current.technique;
    if (blocks[1]) blocks[1].innerText = updated.findings   ?? current.findings;
    if (blocks[2]) blocks[2].innerText = updated.impression ?? current.impression;
    if (blocks[3]) blocks[3].innerText = updated.opinions   ?? current.opinions;
