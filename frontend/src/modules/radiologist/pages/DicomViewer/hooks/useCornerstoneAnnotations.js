...
export default function useCornerstoneAnnotations({
  ...
  const saveAnnotationDialog = async (scope) => {
    ...
    // Optimistic local insert with a temp id; replace with server row on success.
    ...
    setDbAnnotations?.((prev) => [optimistic, ...(prev || [])]);
    ...
    if (!isUuid || !backendUrl) {
      const reason = !isUuid
        ? `userId is not a UUID (got "${userId}"). Log out and log back in.`
        : "BACKEND_URL is empty.";
      console.warn("[Annotations] Save to DB skipped:", reason);
      try { window.alert(`Annotation NOT saved to DB.\nReason: ${reason}`); } catch {}
      return;
    }

    console.log("[Annotations] POST", `${backendUrl}/radiology/annotations`, {
      case_id: caseId, user_id: userId, visibility, annotation_type: safeType,
    });

    try {
      const res = await fetch(`${backendUrl}/radiology/annotations`, {
        method: "POST",
      ...
      console.log("[Annotations] POST result", res.status, json || rawText.slice(0, 200));
      if (!res.ok || !json?.success || !json?.data) {
        throw new Error(json?.detail || `HTTP ${res.status}: ${rawText.slice(0, 200)}`);
      }
      setDbAnnotations?.((prev) =>
        (prev || []).map((x) => (x.annotation_id === tempId ? json.data : x))
      );
    } catch (e) {
      console.error("[Annotations] Save to DB failed; rolling back:", e);
      try { window.alert(`Annotation NOT saved to DB.\nError: ${e?.message || e}`); } catch {}
      setDbAnnotations?.((prev) =>
        (prev || []).filter((x) => x.annotation_id !== tempId)
      );
      setSavedCornerstoneUids?.((prev) => {
        const next = new Set(prev);
        next.delete(uid);
