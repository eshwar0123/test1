...
export const toSameOriginUrl = (url) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        /* Decision tree:
           1. FIRST load → always single-viewport, regardless of series size.
              Doctors open into 1×1 so every panel shows the correct, distinct
              series from the first frame; 3-up MPR / grid is opt-in via the
              toolbar, not auto-selected.
        ...
        const shouldUseMain2 = showTri && !isFirstLoad && currentMode === "main2";
        console.log(
          `[useViewerDataLoader] isFirstLoad=${isFirstLoad} currentMode=${currentMode} ` +
          `showTri=${showTri} → shouldUseMain2=${shouldUseMain2}`
        );
        if (shouldUseMain2) {
          // shouldUseMain2 only fires on reloads where the grid is already
          // main2 (preserving the user's own layout choice) — first load
          // always takes the single-viewport branch below.
          // PERF: poll for layout readiness instead of sleeping 500ms blind.
          const ok = await waitForLayout(
            () => [axRef.current, sagRef.current, corRef.current],
            waitForElementsReady
