...
export default function useCornerstoneNiftiControls({
  ...
  const activateCornerstoneNiftiTool = (mode) => {
    ...
    // "none" or any unrecognised mode: leave all tools passive so the cursor is the default arrow
  ...
  const handleCornerstoneNiftiWheel = (slot, e) => {
    ...
    if (niftiSyncEnabled) {
      const visibleSlots = getVisibleNiftiSlots();
      visibleSlots.forEach((s) => scrollCornerstoneNiftiBySlot(s, dir));
    } else {
  ...
  const rebuildNiftiToolGroup = (viewportIds) => {
    ...
    // Per-plane colors matching the canvas crosshair overlay palette:
    // NIFTI_SLOT_0 = Axial → blue, NIFTI_SLOT_1 = Sagittal → orange, NIFTI_SLOT_2 = Coronal → green
    const PLANE_COLORS = { "NIFTI_SLOT_0": "#378ADD", "NIFTI_SLOT_1": "#EF9F27", "NIFTI_SLOT_2": "#97C459" };
    safeAddTool(tg, CrosshairsTool.toolName, {
      configuration: {
        getReferenceLineColor: (vpId) => PLANE_COLORS[vpId] || "#22c55e",
