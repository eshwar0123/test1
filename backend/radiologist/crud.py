...
# Radiologist helpers
# -----------------------------
def _ensure_dmc_no_column(conn: connection) -> None:
    """Lazily adds radiologists.dmc_no if it doesn't exist yet — same
    self-healing pattern as _ensure_availability_pk, so no separate
    migration step is needed to roll this field out."""
    with conn.cursor() as cur:
        cur.execute(
            "ALTER TABLE radiology_schema.radiologists "
            "ADD COLUMN IF NOT EXISTS dmc_no TEXT"
        )
    conn.commit()
...
def update_dmc_no(conn: connection, user_id: UUID, dmc_no: str) -> None:
    _ensure_dmc_no_column(conn)
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE radiology_schema.radiologists
            SET dmc_no=%s, updated_at=NOW()
            WHERE user_id=%s
            """,
            (dmc_no, str(user_id))
        )
...
def update_designation(conn: connection, user_id: UUID, designation: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE radiology_schema.radiologists
            SET designation=%s, updated_at=NOW()
            WHERE user_id=%s
            """,
            (designation, str(user_id))
        )
    conn.commit()
