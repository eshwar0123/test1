...
_EDITOR_SYSTEM_PROMPT = (
    ...
    "RULES:\n"
    "1. Apply the instruction only where it logically belongs (e.g. word replacements "
    "apply to all sections; a section-specific instruction applies only to that section).\n"
    "2. Do NOT invent new findings or alter unrelated sections.\n"
    "3. Preserve clinical/radiological tone and medical terminology.\n"
...
def _parse_edited_sections(text: str) -> Dict[str, str]:
    ...
    for m in pattern.finditer(text):
        ...
        if body.lower() in ("(empty)", "(unchanged)"):
            body = ""
        out[key] = body
    return out
...
async def edit_report(data: EditReportRequest):
    ...
    try:
        ...
        logger.error(f"[edit-report] Claude API error: {e}")
        raise HTTPException(status_code=503, detail=f"Claude editor failed: {e}")
    except Exception as e:
        logger.error(f"[edit-report] Claude call failed: {e}")
        raise HTTPException(status_code=503, detail=f"Claude editor failed: {e}")

    sections = _parse_edited_sections(raw)

    # Fallback: if parser found nothing, return raw text in findings so the user sees something
    ...
    # Preserve any section the AI omitted (use the original)
    return {
        "success": True,
        "data": {
            "technique":  sections.get("technique")  or (data.technique  or ""),
            "findings":   sections.get("findings")   or (data.findings   or ""),
            "impression": sections.get("impression") or (data.impression or ""),
            "opinions":   sections.get("opinions")   or (data.opinions   or ""),
        },
    }
