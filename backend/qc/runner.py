...
# Same folder radiologist/router.py mounts at /uploads/thumbnails — computed
# relative to this file rather than importing radiologist.router, to avoid
# coupling this background pipeline to that module's routes/side effects.
THUMB_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "radiologist", "uploads", "thumbnails")
...
def run_qc_for_case(case_meta: dict) -> None:
    ...
    try:
        ...
        # Render a preview thumbnail into the SAME folder radiologist/router.py's
        # own uploads already serve at /uploads/thumbnails — so the worklist
        # shows a real preview instead of a placeholder, with no new static
        # mount needed. Must happen before the S3-upload step below, which
        # deletes the local files once they're safely in S3.
        thumb_filename = None
        if img_dir_a and filenames:
            try:
                thumb_filename = engine.generate_case_thumbnail(
                    image_dir=img_dir_a,
                    filenames=filenames,
                    output_folder=THUMB_DIR,
                    base_name=case_id or upload_id,
                )
            except Exception as thumb_err:
                print(f"[runner] thumbnail generation failed for {case_id}: {thumb_err}")
                    ...
                    if rad:
                        ...
                        if wf_id:
                            ...
                            try:
                                ...
                                # Bulk-upload path never goes through /scans/{case_id}/assign —
                                # fire the same new-case notification/push here instead.
                                try:
                                    rad_user_id = rad.get("rad_user_id") or rad.get("user_id")
                                    if rad_user_id:
                                        from database import get_conn as _get_conn_notif
                                        from radiologist.router import _create_notification
                                        _notif_conn = _get_conn_notif()
                                        try:
                                            with _notif_conn.cursor() as _notif_cur:
                                                _create_notification(
                                                    _notif_cur, rad_user_id, case_id,
                                                    "New Case Assigned",
                                    ...
                                    print(f"[notifications] failed to notify on bulk-upload assign: {_notif_err}")
