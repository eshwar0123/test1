...
# ══════════════════════════════════════════════════════════════════════════════
# CASE THUMBNAIL — generated at upload/QC time so the radiologist's worklist
# shows a real preview instead of a placeholder icon. Walks the case's files
# from the middle outward (mirrors radiologist/router.py's
# generate_dicom_series_thumbnail) since some slices in a series lack pixel
# data; returns as soon as one file renders successfully.
# ══════════════════════════════════════════════════════════════════════════════
def _save_thumb_from_pixels(arr, out_path, size=(384, 384)) -> bool:
    if not (NUMPY and PILLOW) or arr is None:
        return False
    try:
        a = np.asarray(arr)
        if a.ndim == 3:
        ...
        if normed is None:
            return False
        img = PILImage.fromarray(normed.astype(np.uint8))
...
def generate_case_thumbnail(image_dir: str, filenames: List[str], output_folder: str, base_name: str) -> Optional[str]:
    """Render one representative slice from the case's files to a PNG in
       output_folder, named "{base_name}_thumb.png". Returns the bare
       filename (not a path) on success, None if nothing rendered."""
    if not filenames:
        return None
    ...
    for off in range(1, n):
        if mid - off >= 0: order.append(mid - off)
        if mid + off < n:  order.append(mid + off)

    for idx in order:
        fpath = os.path.join(image_dir, filenames[idx])
        lower = fpath.lower()
        try:
            if lower.endswith(".dcm") and PYDICOM:
                ds = pydicom.dcmread(fpath, force=True)
                arr = ds.pixel_array
                slope = float(getattr(ds, "RescaleSlope", 1) or 1)
                intercept = float(getattr(ds, "RescaleIntercept", 0) or 0)
                arr = np.asarray(arr, dtype=np.float32) * slope + intercept
                if _save_thumb_from_pixels(arr, out_path):
                    return thumb_filename
            elif (lower.endswith(".nii") or lower.endswith(".nii.gz")) and NIBABEL:
                vol = nib.load(fpath).get_fdata()
                if vol.ndim == 4:
                    vol = vol[:, :, :, 0]
                mid_slice = vol.shape[2] // 2
                slice_data = np.rot90(vol[:, :, mid_slice])
                if _save_thumb_from_pixels(slice_data, out_path):
                    return thumb_filename
        except Exception:
            continue
    return None
